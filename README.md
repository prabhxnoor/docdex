...
| search + read each top file | 11/11 | n/a (would guess) | 20,176 |
| **`docdex context --from-file`** | **9/11** | **1/1** | **1,595** |

The naive search loop *can* cover everything — by reading 20k tokens of full documents, and with no way to say "this field isn't in the corpus." `docdex context` delivers ~82% of the findable fields at ~8% of that token cost **and** correctly reports the absent field as not found instead of forcing a guess. The packet carries a coverage header (*found / weak / missing / dropped-by-budget*), a token-exact budget line, and a conflicts section, so an agent can never mistake a partial packet for a complete one. As of **v0.4.0** the answers are also *faithful*: each field's value is extracted from the window right after that field's own label (never a neighbour's value on a dense line), equivalent amounts no longer false-conflict, and a present fact is never reported "missing" just because its relevance score rounds to zero — the trust failures an independent audit surfaced in v0.3. **v0.5.0** added meaning-aware search (stemming, declared synonyms, a utility reranker) and **v0.5.1** repaired a precision cost that came with it: stemming had been flattening a term's literal form into its stem class, so a word that appeared in exactly one chunk inherited the ranking weight of one appearing in 154 — burying the passage that held the answer. The text is now indexed both as written and stemmed, and each passage keeps its stronger match, so stemming can only *add* findable evidence. That is what moved this benchmark 8/11 → 9/11 at slightly fewer tokens. The 2 fields still missed are honest limits: a field labelled `Legal name` won't read its value from a clause that says `Vendor`, because *form-field* synonym/inflection matching is the next release (free-text search already bridges both). It does not fabricate them; it lists them under "Missing."
