...
|---|---|---|---|
| read whole files until budget | 0/11 | n/a | 351 (then stops) |
| search + read each top file | 11/11 | n/a (would guess) | 20,096 |
| **`docdex context --from-file`** | **11/11** | **1/1** | **1,424** |

The naive search loop *can* cover everything — by reading 20k tokens of full documents, and with no way to say "this field isn't in the corpus." `docdex context` delivers **all** of the findable fields at ~7% of that token cost **and** correctly reports the absent field as not found instead of forcing a guess. The packet carries a coverage header (*found / weak / missing / dropped-by-budget*), a token-exact budget line, and a conflicts section, so an agent can never mistake a partial packet for a complete one. As of **v0.4.0** the answers are also *faithful*: each field's value is extracted from the window right after that field's own label (never a neighbour's value on a dense line), equivalent amounts no longer false-conflict, and a present fact is never reported "missing" just because its relevance score rounds to zero — the trust failures an independent audit surfaced in v0.3. **v0.5.0** added meaning-aware search (stemming, declared synonyms, a utility reranker) and **v0.5.1** repaired a precision cost that came with it: stemming had been flattening a term's literal form into its stem class, so a word that appeared in exactly one chunk inherited the ranking weight of one appearing in 154 — burying the passage that held the answer. The text is now indexed both as written and stemmed, and each passage keeps its stronger match, so stemming can only *add* findable evidence. **v0.5.2** then extended that to form fields themselves: a field called `Governing law` reads its value from "governed by the laws of…", and one called `Legal name` reads from a clause saying `Vendor` if you declared that synonym — anything matched loosely is flagged `~approx`. It also stopped sixty documents that merely *repeat* a field's label from burying the one that actually answers it. Together those moved this benchmark 8/11 → 9/11 → 10/11, at fewer tokens each time. **v0.5.7** closed the last miss and reads **11/11**: contracts often name a party as "Helios Components Pvt Ltd **as the Vendor**", where the value sits *before* the label, and reading backwards is the direction that leaks a neighbour's value, so it is fenced in tightly — a run of proper nouns ending in a legal form, immediately before a required connective, for a field that wants a party. **v0.5.8** then made the other direction work too (`Legal name: Beta Holdings Ltd` was labelled and unreadable, because a company name matches no number, amount or date pattern), stopped a document cross-reference being presented as a value ("Liability cap: refer to section 9" was an *answer*), and stopped a value losing its unit (`Renewal term: 24` — 24 what?).
